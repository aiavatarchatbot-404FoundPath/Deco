// app/api/chat/route.ts
  ...
  // ---------------- Conversations: ensure & summary ----------------
  async function ensureConversationRow(conversationId: string) {
    const { data, error } = await supabase
      .from("conversations")
      .select("id")
    ...
    if (!data) {
      const { error: insErr } = await supabase
        .from("conversations")
        .insert({ id: conversationId, summary: "" });
  ...
  async function saveTurnToDB({
    ...
    const rows: any[] = [];
    // ALWAYS store the user turn (sender_id nullable or use a fixed ANON ID)
    rows.push({
  ...
  export async function POST(req: Request) {
    try {
      ...
      await ensureConversationRow(conversationId);
      ...
      // 0) Hard-stop regex → immediate escalation (fast path)
  ...
  Return plain text suitable for a chat UI.`;
      ...
      // 5) Persist the new user+assistant turn (always insert both rows)
      await saveTurnToDB({
        conversationId,
        userId,
        botUserId: BOT_USER_ID,
      ...
      // 6) Refresh summary:
      // - Refresh on each N-th assistant message
      // - Also force an initial summary if it's empty
